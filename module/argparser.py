...
class ArgParser(object):
    def __init__(self):
        self.parser = argparse.ArgumentParser(description="BERT content based Recommendation Model Arguments Parser.")
        ...
        self.parser.add_argument(
            "--embedding_path",
            type=str,
            help="Directory path to store embedding json. (e.g. embs.json)",
        )
        self.parser.add_argument(
            "-v",
            "--valid",
            action="store_true",
            help="Whether to split data to train and validation set.",
        )
        self.parser.add_argument(
            "--split_ratio", type=float, default=0.3, help="Split ratio for train test split."
        )
        self.parser.add_argument(
            "-rd", "--remove_duplicate", action="store_true", help="Remove duplicate instances."
        )
